...
class BackupCreate(BaseModel):
    collections: Optional[List[str]] = Field(None, description="Specific collections to back up; None means all")
    description: Optional[str] = Field(None, max_length=500, description="Human-readable backup description")
...
class BackupSchedule(BaseModel):
    cron_expression: str = Field(..., min_length=5, description="Cron expression, e.g. '0 2 * * *'")
    collections: Optional[List[str]] = Field(None, description="Collections to include; None means all")
