...
_VALID_PGP_TYPES = ("PGP PRIVATE KEY BLOCK", "PGP PUBLIC KEY BLOCK", "PGP PRIVATE KEY", "PGP PUBLIC KEY")
...
def validate_gpg_key(key: str) -> Dict:
    ...
    if pgp_type not in _VALID_PGP_TYPES:
        return {
            "status": "invalid",
            "response_time": 0,
            "message": f"Unrecognized PGP block type: '{pgp_type}'. "
                       f"Expected one of: {', '.join(_VALID_PGP_TYPES)}",
        }
