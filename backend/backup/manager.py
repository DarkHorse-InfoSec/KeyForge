...
class BackupManager:
    ...
    async def create_backup(
        ...
        await db["backup_data"].insert_one({"backup_id": backup_id, "data": b64_data})
    ...
    async def restore_backup(
        ...
        cols_to_restore = (
            [c for c in target_collections if c in payload] if target_collections else list(payload.keys())
        )
    ...
    async def get_schedule(db) -> Optional[dict]:
        ...
        doc = await db["backup_schedules"].find_one({"enabled": True}, {"_id": 0})
        return doc
