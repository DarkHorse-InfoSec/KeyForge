...
try:
    ...
    from ..security import JWT_ALGORITHM, JWT_SECRET, decrypt_api_key
except ImportError:
    from backend.config import db, logger
    from backend.security import JWT_ALGORITHM, JWT_SECRET, decrypt_api_key
...
class ProxyTokenManager:
    ...
    async def validate_proxy_token(self, token: str) -> dict:
        ...
        record = await db[self.PROXY_TOKEN_COLLECTION].find_one({"token_id": token_id})
        if record is None:
    ...
    async def proxy_request(
        ...
        if allowed:
            if not _url_matches_any(target_url, allowed):
                raise ValueError("Target URL is not in the allowed endpoints for this proxy token")
