...
async def list_groups(
    ...
    personal = await db.credential_groups.find({"user_id": current_user["id"]}).to_list(None)

    # Team groups: find all teams the user belongs to, then find team groups
    memberships = await db.team_members.find({"user_id": current_user["id"]}).to_list(None)
...
async def add_credential_to_group(
    ...
    return {
        "message": "Credential added to group",
