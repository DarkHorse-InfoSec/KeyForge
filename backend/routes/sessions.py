...
async def list_sessions(
    ...
    sessions = await db.sessions.find({"user_id": current_user["id"], "is_active": True}).to_list(length=500)
...
async def revoke_session(
    ...
    session = await db.sessions.find_one({"id": session_id, "user_id": current_user["id"], "is_active": True})
