...
async def grant_permission(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Permission already exists for this user on this credential",
        )
...
@router.get(
    "/credential-permissions/{credential_id}",
    response_model=List[CredentialPermissionResponse],
)
async def list_permissions(
    ...
    permissions = await db.credential_permissions.find({"credential_id": credential_id}).to_list(1000)
...
@router.get(
    "/credential-permissions/shared-with-me",
    response_model=List[CredentialPermissionResponse],
)
async def shared_with_me(
    ...
    permissions = await db.credential_permissions.find({"user_id": current_user["id"]}).to_list(1000)
...
@router.get(
    "/credential-permissions/my-shares",
    response_model=List[CredentialPermissionResponse],
)
async def my_shares(
    current_user: dict = Depends(get_current_user),
    ...
    permissions = await db.credential_permissions.find({"granted_by": current_user["id"]}).to_list(1000)
