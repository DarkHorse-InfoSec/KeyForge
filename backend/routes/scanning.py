...
async def scan_dependencies(
    ...
    user_credentials = await db.credentials.find({"user_id": current_user["id"]}).to_list(length=500)
...
async def scan_history(
    ...
    records = await db.scan_results.find({"user_id": current_user["id"]}).sort("timestamp", -1).to_list(length=100)
