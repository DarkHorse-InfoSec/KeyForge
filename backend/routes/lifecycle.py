...
try:
    ...
    from ..models_analytics import LifecycleTimelineResponse
    from ..security import get_current_user
except ImportError:
    from backend.config import db
    from backend.models_analytics import LifecycleTimelineResponse
...
async def get_credential_timeline(
    ...
    events = (
        await db.lifecycle_events.find({"credential_id": credential_id, "user_id": current_user["id"]})
        .sort("timestamp", 1)
        .to_list(1000)
    )
...
async def get_recent_lifecycle_events(
    ...
    events = await db.lifecycle_events.find({"user_id": current_user["id"]}).sort("timestamp", -1).limit(50).to_list(50)

    # Enrich with credential api_name
    cred_ids = list({e["credential_id"] for e in events})
    credentials = await db.credentials.find({"id": {"$in": cred_ids}, "user_id": current_user["id"]}).to_list(1000)
