...
async def configure_auto_rotation(
    ...
    if existing:
        raise HTTPException(status_code=400, detail="Auto-rotation config already exists for this credential")
...
async def list_auto_rotation_configs(
    ...
    configs = await db.auto_rotation_configs.find(
        {"user_id": current_user["id"]}
    ).to_list(1000)
...
async def get_supported_providers(current_user: dict = Depends(get_current_user)):
    ...
    return {
        "providers": [
            {"key": key, **details}
            for key, details in SUPPORTED_PROVIDERS.items()
        ]
    }
