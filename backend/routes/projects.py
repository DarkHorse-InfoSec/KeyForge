...
try:
    ...
    from ..models import ProjectAnalysis, ProjectCreate
    from ..patterns import analyze_code_content
    from ..security import get_current_user
except ImportError:
    from backend.config import db
    from backend.models import ProjectAnalysis, ProjectCreate
    from backend.patterns import analyze_code_content
...
async def get_project_analyses(
    ...
    analyses = await db.project_analyses.find({"user_id": current_user["id"]}).skip(skip).limit(limit).to_list(limit)
...
async def upload_project_files(
    ...
    for api in unique_apis.values():
        recommendations.append(f"Configure {api['name']} credentials for {api['category']} integration")
