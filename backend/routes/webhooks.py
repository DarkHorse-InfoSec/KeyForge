...
async def trigger_webhooks(
    ...
    """
    webhooks = await db_ref.webhooks.find({"user_id": user_id, "enabled": True, "events": event}).to_list(100)
...
async def create_webhook(
    ...
    if invalid:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid event(s): {', '.join(invalid)}. " f"Valid events: {', '.join(VALID_EVENTS)}",
        )
