...
async def get_key_status(
    ...
    key_doc = await db.user_data_keys.find_one({"user_id": user_id, "is_active": True})
...
async def rotate_master_key(
    ...
    return MasterKeyRotationResponse(
        message=(
            "Master key rotated successfully. " "Update the ENCRYPTION_KEY environment variable and restart the server."
        ),
        data_keys_re_wrapped=result["data_keys_re_wrapped"],
        rotated_at=result["rotated_at"],
    )
