...
try:
    ...
    from ..models_analytics import BreachCheckResponse
    from ..security import decrypt_api_key, get_current_user
except ImportError:
    from backend.config import db
    from backend.models_analytics import BreachCheckResponse
...
async def _check_hash_collision(key_hash: str, user_id: str, credential_id: str) -> str | None:
    ...
    all_credentials = await db.credentials.find({"user_id": {"$ne": user_id}}).to_list(1000)
...
async def get_breach_check_results(
    ...
    results = await db.breach_checks.find({"user_id": current_user["id"]}).to_list(1000)
...
async def get_breach_check_summary(
    ...
    checks = await db.breach_checks.find({"user_id": current_user["id"]}).sort("check_timestamp", -1).to_list(1000)
