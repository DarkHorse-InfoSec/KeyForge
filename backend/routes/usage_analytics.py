...
try:
    ...
    from ..models_analytics import UsageEvent, UsageAnalytics
except ImportError:
    from backend.config import db, logger
    from backend.security import get_current_user
    from backend.models_analytics import UsageEvent, UsageAnalytics
...
async def _compute_analytics_for_credential(credential: dict, user_id: str) -> dict:
    ...
    pipeline = [
        ...
        {"$group": {
            ...
            "uses_last_7_days": {
                "$sum": {"$cond": [{"$gte": ["$timestamp", seven_days_ago]}, 1, 0]}
            },
            "uses_last_30_days": {
                "$sum": {"$cond": [{"$gte": ["$timestamp", thirty_days_ago]}, 1, 0]}
            },
        }},
    ]
