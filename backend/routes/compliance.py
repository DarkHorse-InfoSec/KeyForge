    ...
    findings.append(
        {
            ...
            "finding": (
                f"Encryption: {cred_analysis['encryption_algorithm']}"
                if cred_analysis["all_encrypted"]
                else "Not all data is encrypted at rest"
            ),
            "recommendation": ("" if cred_analysis["all_encrypted"] else "Encrypt all stored credentials"),
        }
    )
    ...
    findings.append(
        {
            ...
            "finding": (
                "MFA enabled for access control"
                if security_analysis["mfa_enabled"]
                else "MFA not enabled; access controls may be insufficient"
            ),
            "recommendation": ("" if security_analysis["mfa_enabled"] else "Enable multi-factor authentication"),
        }
    )
    ...
    findings.append(
        {
            ...
            "finding": (
                f"Audit trail contains {audit_analysis['total_audit_entries']} entries"
                if audit_analysis["logging_active"]
                else "No processing activity records found"
            ),
            "recommendation": (
                "" if audit_analysis["logging_active"] else "Implement audit logging for all data processing activities"
            ),
        }
    )
    ...
    findings.append(
        {
            ...
            "status": "pass" if cred_analysis["all_encrypted"] else "fail",
            "finding": (
                f"All credentials encrypted ({cred_analysis['encryption_algorithm']})"
                if cred_analysis["all_encrypted"]
                else "Some credentials are not encrypted"
            ),
        }
    )
    ...
    findings.append(
        {
            ...
            "finding": (
                f"{security_analysis['ip_allowlist_count']} IPs in allowlist"
                if security_analysis["ip_allowlisting"]
                else "No IP allowlisting configured"
            ),
        }
    )
...
async def list_compliance_reports(
    ...
    reports = await db.compliance_reports.find({"user_id": current_user["id"]}).sort("generated_at", -1).to_list(1000)
...
async def get_compliance_score(
    ...
    if cred_analysis["rotation_policy_pct"] >= 100:
        ...
        breakdown.append(
            {
                "item": "All credentials have rotation policy",
                "points": 15,
                "earned": True,
            }
        )
    ...
    if cred_analysis["expiration_tracking_pct"] >= 100:
        ...
        breakdown.append(
            {
                "item": "Expiration tracking on all credentials",
                "points": 15,
                "earned": True,
            }
        )
    ...
    return {
        ...
        "max_score": 100,
        "grade": (
            "A" if score >= 90 else ("B" if score >= 75 else "C" if score >= 60 else "D" if score >= 40 else "F")
        ),
        "breakdown": breakdown,
    }
