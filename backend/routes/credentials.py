...
try:
    ...
    from ..models import CredentialCreate, CredentialResponse, CredentialUpdate
    from ..security import decrypt_api_key, encrypt_api_key, get_current_user
    from ..validators import validate_credential
except ImportError:
    from backend.config import db
    from backend.models import CredentialCreate, CredentialResponse, CredentialUpdate
...
async def get_credentials(
    ...
    credentials = await db.credentials.find({"user_id": current_user["id"]}).skip(skip).limit(limit).to_list(limit)
