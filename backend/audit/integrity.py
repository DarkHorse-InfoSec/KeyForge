...
class AuditIntegrity:
    ...
    async def create_audit_entry(
        ...
        previous_hash = last_entry["integrity_hash"] if last_entry else cls.GENESIS_HASH
    ...
    async def verify_chain(
        ...
        entries = await db.audit_log.find(query).sort("timestamp", 1).limit(limit).to_list(limit)
    ...
    async def export_audit_log(
        ...
        entries = await db.audit_log.find(query).sort("timestamp", 1).to_list(10000)
