...
class EnvelopeEncryption:
    ...
    async def _get_or_create_user_key(self, user_id: str) -> dict:
        ...
        """
        doc = await db.user_data_keys.find_one({"user_id": user_id, "is_active": True})
        if doc:
            return doc
    ...
    async def rotate_user_data_key(self, user_id: str) -> dict:
        ...
        """
        old_key_doc = await db.user_data_keys.find_one({"user_id": user_id, "is_active": True})
        ...
        if old_key_doc:
            ...
            credentials = await db.credentials.find({"user_id": user_id}).to_list(length=10_000)
            ...
            for cred in credentials:
                ...
                try:
                    plaintext = self.decrypt_with_data_key(envelope_data["ciphertext"], old_data_key)
                    new_ciphertext = self.encrypt_with_data_key(plaintext, new_plaintext_key)
    ...
    async def rotate_master_key(self, new_master_key: str) -> dict:
        ...
        Returns summary info about the rotation.
        """
        new_master_fernet = Fernet(new_master_key if isinstance(new_master_key, bytes) else new_master_key.encode())

        all_keys = await db.user_data_keys.find({"is_active": True}).to_list(length=100_000)
        ...
        self._master_key = new_master_key if isinstance(new_master_key, bytes) else new_master_key.encode()
        self._master_fernet = Fernet(self._master_key)

        logger.info("Master key rotated - re-wrapped %d user data keys", re_wrapped_count)
