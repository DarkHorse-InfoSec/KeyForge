...
class AWSKMSProvider(KMSProvider):
    ...
    def __init__(self) -> None:
        try:
            ...
            raise RuntimeError("boto3 is required for the AWS KMS provider. " "Install it with: pip install boto3")
...
class VaultKMSProvider(KMSProvider):
    ...
    def __init__(self) -> None:
        try:
            ...
            raise RuntimeError("httpx is required for the Vault KMS provider. " "Install it with: pip install httpx")

        self._addr = os.environ.get("VAULT_ADDR")
        self._token = os.environ.get("VAULT_TOKEN")
        if not self._addr or not self._token:
            raise RuntimeError(
                "VAULT_ADDR and VAULT_TOKEN environment variables are required " "for the Vault KMS provider"
            )
...
def get_kms_provider() -> KMSProvider:
    ...
    if cls is None:
        raise RuntimeError(f"Unknown KMS_PROVIDER '{name}'. " f"Supported values: {', '.join(_PROVIDERS)}")
